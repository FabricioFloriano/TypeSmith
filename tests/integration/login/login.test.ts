...
describe('POST /login', function () { 
  ...
  it('Testa o login', async function () {
  ...
  it('Testa o login com senha incorreta', async function () {
    const mockFindTwo = UserModel.build({id: 1, username: 'Helga', vocation: 'Curandeira', level: 9, password: 'ruan'})
    sinon.stub(UserModel, 'findOne').resolves(mockFindTwo);
    const Response = await chai.request(app).post('/login').send({username: 'Helga', password: 'valquíria'});
